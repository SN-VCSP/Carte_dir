...
with st.sidebar:
    ...
    with st.expander("🧭 Sélection du segment", expanded=True):
        if _df_f.empty:
            ...
            cote_labels = {"D": "Sens 1", "G": "Sens 2"}
            cote = st.selectbox(
                "Sens",
                sorted(cotes_dispo),
                format_func=lambda v: cote_labels.get(str(v), str(v)),
            )
            ...
            if pr_end is not None:
                seg_points = subset[
                    (subset["pr"] >= pr_start) & (subset["pr"] <= pr_end)
                ].copy()
                seg_points = seg_points.sort_values("pr").reset_index(drop=True)
                if seg_points.empty:
                    st.info("Aucun point disponible pour ce segment.")
                else:
                    z_candidates = [
                        col
                        for col in ["z", "Z", "altitude", "Altitude", "alt", "ALT"]
                        if col in seg_points.columns
                    ]
                    z_col = z_candidates[0] if z_candidates else None
                    lines = []
                    point_idx = 1
                    for row in seg_points.itertuples(index=False):
                        x_val = getattr(row, "x", None)
                        y_val = getattr(row, "y", None)
                        ...
                        if z_col:
                            ...
                            z_f = 0.0
                        lines.append(f"{point_idx} {x_f:.3f} {y_f:.3f} {z_f:.3f}")
                        point_idx += 1
                    if lines:
                        def _slug_local(value: Any) -> str:
                            allowed = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_-"
                            raw = str(value).strip().replace(" ", "_")
                            return "".join(ch if ch in allowed else "_" for ch in raw)
                        try:
                            pr_start_part = int(float(pr_start))
                            pr_end_part = int(float(pr_end))
                            pr_label = f"PR{pr_start_part}-{pr_end_part}"
                        except Exception:
                            pr_label = "segment"
                        file_name = f"points_{_slug_local(route)}_{_slug_local(cote)}_{pr_label}.txt"
                        txt_content = "\n".join(lines)
                        st.download_button(
                            "Exporter les points du segment (TXT)",
                            data=txt_content,
                            file_name=file_name,
                            mime="text/plain",
                            help="Format: ID X Y Z (metres)"
                        )
                    else:
                        st.info("Aucun point exploitable pour ce segment.")

                    def _extract_xy(row: pd.Series) -> Optional[Tuple[float, float]]:
                        x_val = getattr(row, "x", None)
                        y_val = getattr(row, "y", None)
                        if pd.isna(x_val) or pd.isna(y_val):
                            return None
                    ...
                    start_xy = _extract_xy(seg_points.iloc[0])
                    end_xy = _extract_xy(seg_points.iloc[-1])
                    maps_selection_key = (
                        str(route),
                        str(cote),
                        str(pr_start),
                    ...
                    if start_xy and end_xy:
                        if st.button(
                            "Generer les liens Maps (PR debut/fin)",
                            key="btn_maps_links",
                            help="Cree des liens directs Google Maps pour les PR debut et fin."
                        ):
                            start_wgs, end_wgs = l93_to_wgs([start_xy, end_xy])
                            st.session_state["maps_links_state"] = {
                                "key": maps_selection_key,
                                "start_url": f"https://www.google.com/maps?q={start_wgs[0]:.6f},{start_wgs[1]:.6f}",
                                "end_url": f"https://www.google.com/maps?q={end_wgs[0]:.6f},{end_wgs[1]:.6f}",
                            }
                    else:
                        st.session_state.pop("maps_links_state", None)

                    stored_links = st.session_state.get("maps_links_state")
                    if stored_links and stored_links.get("key") == maps_selection_key:
                        st.markdown(
                            f"- [PR debut]({stored_links['start_url']})\n- [PR fin]({stored_links['end_url']})"
                        )
            else:
                st.caption("Choisir un PR fin pour activer l'export de points.")

...
def parse_drawn_polylines(map_data: Optional[Dict[str, Any]]) -> List[List[Tuple[float, float]]]:
    """
    Retourne TOUTES les polylignes (liste de polylignes),
    en gérant LineString, MultiLineString et FeatureCollection.
    Chaque polyligne = liste de (lat, lon).
    """
    if not map_data:
        return []
...
with col_actions:
    ...
    if dist_method == "Segment édité":
        ...
        if st.button("➕ Ajouter comme sous-segment", disabled=not can_add):
            # On prend la dernière polyligne dessinée
            ...
            # Optionnel : vider le tracé temporaire pour éviter les doublons à l'ajout suivant
            st.session_state["edited_geoms"].pop(seg_key, None)
            st.rerun()
    else:
    ...
    if st.session_state["subsegments"].get(seg_key):
        ...
        for idx, it in enumerate(st.session_state["subsegments"][seg_key]):
            ...
            st.write(f"#{idx+1} — {label} — {dist:.2f} m — couleur {it['color']}")

            # ⬇⬇⬇ AJOUT : éditeur des largeurs par sous-segment
            with st.expander(f"Modifier les largeurs pour {label}", expanded=False):
                ...
                # Grille 3 colonnes pour une saisie compacte
                cols = st.columns(3)
